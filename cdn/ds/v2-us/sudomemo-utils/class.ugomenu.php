...
(gauched fucked with this, plz dont replace.)
*/
...
class ugomenu {
  ...
  function addWeird($args) {
    if (!isset($this->menu["weird"])) {
      $this->menu["weird"] = [];
    }
    array_push($this->menu["weird"], [
      "odd"     => isset($args["odd"])     ? $args["odd"]     : ""
    ]);
  }
  ...
  function getUGO(){
    ...
    // TYPE 5 -- UNKNOWN (gauched wantz 2 test o_0)
    if( isset($this->menu["weird"]) ) {
      foreach ($this->menu["weird"] as $item) {
        array_push($menuData, join("\t", [
          "5",
          $item["odd"]
        ]));
