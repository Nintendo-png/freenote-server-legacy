<?php
	...
	$mainMenu->setType("2");
	$mainMenu->setMeta("uppertitle", "Stickfight Channel");
	$mainMenu->setMeta("uppersubtop", "Stickfight stuff.");
	...
	if(isset($_GET['page'])){
		...
		$mainMenu->addButton([
			"label" => "Post Flipnote",
			"url"   => $cdnurl."ds/v2-us/en/channels/7/post.post"
		]);
		...
		foreach ($results as $result) {
			if(substr($result,-3)=="ppm"){
				...
				#$name = str_replace('*&#', "", $name);	
				$mainMenu->addItem([
					"url"   => $cdnurl."ds/v2-us/en/channels/7/".$result,
					"icon"  => "3",
					"counter"=>"0",
					"file"  => $result,
					"fuckoff" => "1"
