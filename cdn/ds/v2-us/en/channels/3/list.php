<?php
	...
	$mainMenu->setType("2");
	$mainMenu->setMeta("uppertitle", "Re-uploads Channel");
	$mainMenu->setMeta("uppersubtop", "Post re-uploads here.");

	$mainMenu->addButton([
		"label" => "Post Flipnote",
		"url"   => $cdnurl."ds/v2-us/en/channels/3/post.post"
	]);
	...
	if(isset($_GET['page'])){
		...
		foreach ($results as $result) {
			if(substr($result,-3)=="ppm"){
				...
				#$name = str_replace('*&#', "", $name);	
				$mainMenu->addItem([
					"url"   => $cdnurl."ds/v2-us/en/channels/3/".$result,
					"icon"  => "3",
					"counter"=>"0",
					"file"  => $result,
					"fuckoff" => "1"
