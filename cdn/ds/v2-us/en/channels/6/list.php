<?php
	...
	$mainMenu->setType("2");
	$mainMenu->setMeta("uppertitle", "Sprite Channel");
	$mainMenu->setMeta("uppersubtop", "Anything sprite-related.");
	...
	if(isset($_GET['page'])){
		...
		$mainMenu->addButton([
			"label" => "Post Flipnote",
			"url"   => $cdnurl."ds/v2-us/en/channels/6/post.post"
		]);
		...
		foreach ($results as $result) {
			if(substr($result,-3)=="ppm"){
				...
				#$name = str_replace('*&#', "", $name);	
				$mainMenu->addItem([
					"url"   => $cdnurl."ds/v2-us/en/channels/6/".$result,
					"icon"  => "3",
					"counter"=>"0",
					"file"  => $result,
					"fuckoff" => "1"
