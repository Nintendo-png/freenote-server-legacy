<?php
	...
	$mainMenu->setType("2");
	$mainMenu->setMeta("uppertitle", "Audio Channel");
	$mainMenu->setMeta("uppersubtop", "Upload your audio here!");

	$mainMenu->addButton([
		"label" => "Post Flipnote",
		"url"   => $cdnurl."ds/v2-us/en/channels/2/post.post"
	]);
	...
	if(isset($_GET['page'])){
		...
		foreach ($results as $result) {
			if(substr($result,-3)=="ppm"){
				...
				#$name = str_replace('*&#', "", $name);	
				$mainMenu->addItem([
					"url"   => $cdnurl."ds/v2-us/en/channels/2/".$result,
					"icon"  => "3",
					"counter"=>"0",
					"file"  => $result,
					"fuckoff" => "1"
