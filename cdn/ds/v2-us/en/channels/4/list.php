<?php
	...
	$mainMenu->setType("2");
	$mainMenu->setMeta("uppertitle", "General Channel");
	$mainMenu->setMeta("uppersubtop", "Upload... things!");

	$mainMenu->addButton([
		"label" => "Post Flipnote",
		"url"   => $cdnurl."ds/v2-us/en/channels/4/post.post"
	]);
	...
	if(isset($_GET['page'])){
		...
		$mainMenu->addItem([
			"url"   => $cdnurl."ds/v2-us/en/channels/4/BEED0E_3EC66DE55247B_358.ppm",
			"icon"  => "3",
			"counter"=>"911",
			"file"  => "BEED0E_3EC66DE55247B_358.ppm",
			"fuckoff" => "1"
		]);
		...
		foreach ($results as $result) {
			if(substr($result,-3)=="ppm"){
				...
				#$name = str_replace('*&#', "", $name);	
				$mainMenu->addItem([
					"url"   => $cdnurl."ds/v2-us/en/channels/4/".$result,
					"icon"  => "3",
					"counter"=>"0",
					"file"  => $result,
					"fuckoff" => "1"
